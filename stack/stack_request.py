...
class StackRequest:
    ...
    def __init__(self, endpoint, key, max_pages):
        ...
        self.site.page_size = 2

    def run_query_question(self, endpoint, min, sort, filter, from_date, until_date):
        return self.site.fetch(endpoint=endpoint, sort=sort, filter=filter, fromdate=str(datetime.datetime.timestamp(from_date)).replace(".0", ""), todate=str(datetime.datetime.timestamp(until_date)).replace(".0", ""))
